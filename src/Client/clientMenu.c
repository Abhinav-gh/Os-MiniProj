...
void BorrowerMenuPrinter()
{
    ...
    printf("5.) Pay fine\n");
    printf("6.) View all borrowed books\n");
    printf("99.) Logout\n");
...
void removeBookHelper(int sock)
{
...
void addBorrowerHelper(int sock)
{
...
void BorrowerMenu(int sock)
{
    ...
    while (choice != 99)
    {
        ...
        case 5:
            ...
            break;
        case 6:
            send(sock, "viewBorrowedBooks", strlen("viewBorrowedBooks"), 0);
            usleep(100000);
            send(sock, clientUsername, strlen(clientUsername), 0);
            break;
        case 99:
...
// ------------->>>>> Finished Important functionalities  <<<<<<<<<<<<------------
